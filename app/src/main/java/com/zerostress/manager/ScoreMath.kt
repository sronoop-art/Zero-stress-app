 ...
 * Both writers (SubmitMatchActivity = player-submitted, DailyInputActivity =
 * admin-logged) MUST use these helpers so a player's lifetime `score`,
 * the daily/weekly/monthly leaderboard buckets, and the match-log charts
 * always move together and match each other.
 *
...
object ZsScore {
    ...
    /** Coins for a player-submitted match. */
    fun entryCoins(kills: Int, isWin: Boolean): Long =
        kills.toLong() * 2L + if (isWin) 25L else 5L

    /** Coins for an admin-logged daily entry (70% of the player-submitted rate). */
